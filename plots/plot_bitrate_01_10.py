...
def save_figure(data, file_name):
    ...
    #--Plot
    sns_plot = sns.lineplot(x=datafile_col_x,y=datafile_col_y4,
                            marker='*',markersize=14,color="blue",label=datafile_col_y4,
                            data=data)


    ax1 = sns_plot.axes
    ax2 = sns_plot.axes.twinx()
    sns_plot = sns.lineplot(x=datafile_col_x,y=datafile_col_y5, ax=ax2,
                            marker='^', markersize=10,color="red", label=datafile_col_y5,
                            data=data)
...
data[datafile_col_y4] = data[datafile_col_y4].str.rstrip('%').astype('float')
data[datafile_col_y5] = data[datafile_col_y5].str.rstrip('%').astype('float')

data1 = data.groupby(by=[datafile_col_x])[datafile_col_y2, datafile_col_y3, datafile_col_y4, datafile_col_y5].agg('min').reset_index()
data2 = data.groupby(by=[datafile_col_x])[datafile_col_y2, datafile_col_y3, datafile_col_y4, datafile_col_y5].agg('mean').reset_index()

save_figure(data1, 'Image Data/bitrate_10_01_min.jpg')
save_figure(data2, 'Image Data/bitrate_10_01_avg.jpg')
