...
datafile_ecc_str = "data/bitrate_ECC_results.txt"
...
def save_figure(data, data1, file_name):
    ...
    sns_plot = sns.lineplot(x=datafile_col_x,y=datafile_col_y2,
                            marker='*',markersize=14,color="blue",label="Bit-Error-Rate without ECC(%)",
                            data=data)


    ax1 = sns_plot.axes
    # ax2 = sns_plot.axes.twinx()
    sns_plot = sns.lineplot(x=datafile_col_x,y=datafile_col_y2, ax=ax1,
                            marker='^', markersize=10,color="red", label="Bit-Error-Rate with ECC(%)",
                            data=data1)
...
#Read Data File using Pandas
ecc_data=pd.read_csv(datafile_ecc_str,sep="\s+")
ecc_data.columns=[datafile_col_x,datafile_col_y1,datafile_col_y2,datafile_col_y4,datafile_col_y5,datafile_col_y6,datafile_col_y7]
ecc_data[datafile_col_y3] = ecc_data[datafile_col_y1].div(8*1024)
#Make percentages to floats:
ecc_data[datafile_col_y2] = ecc_data[datafile_col_y2].str.rstrip('%').astype('float') 
...
ecc_data1 = ecc_data.groupby(by=[datafile_col_x])[datafile_col_y2, datafile_col_y3].agg('min').reset_index()
ecc_data2 = ecc_data.groupby(by=[datafile_col_x])[datafile_col_y2, datafile_col_y3].agg('mean').reset_index()

save_figure(data1, ecc_data1, 'Image Data/bitrate_vs_ecc_min.jpg')
save_figure(data2, ecc_data2, 'Image Data/bitrate_vs_ecc_avg.jpg')
