...
void *thread_start(void *args) {
    ...
    int status;
    CPU_ZERO(&mask);
    CPU_SET(parity+2, &mask);//replaced rx_cpuid with parity + 2
    ...
    fail_if_pthrattr_mismatch(SCHED_FIFO,sched_get_priority_max(SCHED_FIFO),parity+2) ;
        //Start Rx
  ...
  while (1) {
    ...
    if (flip_sequence == 0 && current == 0 && previous == 1) {
 ...
 /////////////////////////// 
  ...
  int parity;
    for (int i = 0; i < THREAD_NUM; ++i) {
        parity = i;
        if (pthread_create(&threads[i], NULL, thread_start, &parity) < 0) {
